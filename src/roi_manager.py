...
class ROIManager:
    ...
    def __init__(self, bin_class_id=0, overlap_threshold=0.1, use_center_point=True, 
                 roi_alpha=0.2, line_thickness=2):
        """初始化ROI管理器"""
        self.roi_mask = None
    ...
    def is_object_in_roi(self, obj: DetectedObject) -> bool:
        ...
        if self.use_center_point:
            # 塑料袋使用宽松的中心点检测
            if obj.get_class_name() == "plastic bag":
                center_result = True  # 塑料袋跳过中心点检查
            else:
    ...
    def _check_overlap_area(self, obj: DetectedObject) -> bool:
        ...
        try:
            ...
            # 塑料袋使用更宽松的重叠阈值
            if obj.get_class_name() == "plastic bag":
                threshold = 0.05  # 塑料袋使用5%阈值（更宽松）
            else:
                threshold = self.overlap_threshold  # 其他类别使用10%阈值
